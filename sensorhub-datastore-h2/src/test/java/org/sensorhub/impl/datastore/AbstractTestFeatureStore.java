 ...
 * @since Apr 14, 2018
 */
public abstract class AbstractTestFeatureStore<StoreType extends IFeatureStore<FeatureKey, AbstractFeature, FeatureFilter>>
{
    ...
    Map<FeatureKey, AbstractFeature> allFeatures = new LinkedHashMap<FeatureKey, AbstractFeature>();
    long timer;
    
    
    protected abstract StoreType initStore(String featureUriPrefix, ZoneOffset timeZone) throws Exception;
    ...
    {
        this.featureStore = initStore(null, ZoneOffset.UTC);
    }
    ...
    {
        String idPrefix = "F";
        if (f instanceof TemporalFeature)
            idPrefix += "T";        
        if (f.isSetGeometry())
            idPrefix += "G";
    ...
    {
        Instant startTime = (f instanceof TemporalFeature) ? 
                ((TemporalFeature)f).getValidTime().lowerEndpoint() :
                Instant.MIN;
        
        return new FeatureKey.Builder()
    ...
    protected void addFeature(AbstractFeature f)
    {
        //System.out.println("Adding " + f.getId());        
        FeatureKey key = getKey(f);        
    ...
    @Test
    public void testGetFeatureUriPrefix() throws Exception
    {
        assertEquals(null, featureStore.getFeatureUriPrefix());
        
        String uriPrefix = "prefix-test";
        this.featureStore = initStore(uriPrefix, ZoneOffset.UTC);
        assertEquals(uriPrefix, featureStore.getFeatureUriPrefix());
    ...
    public void testGetTimeZone() throws Exception
    {
        ...
        this.featureStore = initStore(null, timeZone);
    ...
    private void checkFeatureIDs(Stream<String> featureIds)
    {
        Set<String> datastoreIds = featureIds.collect(Collectors.toSet());
        Set<String> expectedIds = allFeatures.keySet().stream().map(e -> e.getUniqueID()).distinct().collect(Collectors.toSet());
    ...
    private void checkFeaturesEqual(AbstractFeature f1, AbstractFeature f2)
    {
        ...
        if (f1 instanceof TemporalFeature)
            assertEquals(((TemporalFeature)f1).getValidTime(), ((TemporalFeature)f2).getValidTime());
    ...
    {
        System.out.println("\nSelect " + expectedIds + " within " + timeRange);
        ...
        resultMap.forEach((k, v) -> {
            if (!expectedIds.contains(k.getUniqueID()) ||
                (v instanceof TemporalFeature) && !timeRange.isConnected(((TemporalFeature)v).getValidTime()))
                fail("Result contains unexpected feature: " + k);
        });
        
        allFeatures.entrySet().stream()
            .filter(e -> expectedIds.contains(e.getKey().getUniqueID()))
            .filter(e -> !(e.getValue() instanceof TemporalFeature) ||
                         timeRange.isConnected(((TemporalFeature)e.getValue()).getValidTime()))
            .forEach(e -> {
    ...
    @Test
    public void testSelectFeaturesByIdAndTime() throws Exception
    {
        ...
        ids = Sets.newHashSet(UID_PREFIX+"F10", UID_PREFIX+"F31");
        ...
        ids = Sets.newHashSet(UID_PREFIX+"F25", UID_PREFIX+"F49");
    ...
    @Test
    public void testSelectTemporalFeaturesByIdAndTime() throws Exception
    {
        Stream<Entry<FeatureKey, AbstractFeature>> resultStream;
        Set<String> ids;
        Range<Instant> timeRange;
    ...
    public void testGetThroughput() throws Exception
    {
        ...
        for (int i = 0; i < numReads; i++)
        {
            ...
            FeatureKey key = new FeatureKey.Builder()
                    .withUniqueID(uid)
                    .build();
